	...
	public class SpriteTests
	{
		...
		public void PerformingPixelPerfectTest()
		{
			foreach (var sprite in getPixelPerfectImplementors())
			{
				...
				_mocks.MaskLoader().Setup(m => m.Load("Mask_Test", ibitmap, false, null, null)).Returns(mask.Object);
		...
		public void PerformingPixelPerfect_OnlyOnce_Test()
		{
			...
			foreach (var sprite in getPixelPerfectImplementors())
			{
				_mocks.MaskLoader().Setup(m => m.Load("Mask_Test", (IBitmap)null, false, null, null)).Returns((IMask)null);
				sprite.Model.Image = _mocks.Image().Object;

				sprite.PixelPerfect.PixelPerfect(true);
				count++;

				_mocks.MaskLoader().Verify(m => m.Load("Mask_Test", (IBitmap)null, false, null, null), Times.Exactly(count));
			}
		}
		...
		public void DisablingPixelPerfect_WhenEnabled_Test()
		{
			foreach (var sprite in getPixelPerfectImplementors())
			{
				_mocks.MaskLoader().Setup(m => m.Load("Mast_Test", (IBitmap)null, false, null, null)).Returns((IMask)null);                
				sprite.Model.Image = _mocks.Image().Object;
