...
//todo: Currently we can't share the OpenGLESBackend class from Android as we're using the Xamarin dll
//and not the OpenTK dll (had problems with compilation). Once OpenTK is packaged with IOS,
//we should be able to switch, share code with Android and remove this file.

namespace AGS.Engine
{
    public class OpenGLESBackend : IGraphicsBackend
    {
        private Matrix4 _ortho, _view;
        ...
        public void Init()
        {
            ...
            Vector3 eye = new Vector3(0f, 0f, 1f);
        ...
        {
            GL.DrawArrays(getBeginMode(primitiveType), first, count);
        }
        ...
        {
            Matrix4 projection = Matrix4.CreateOrthographicOffCenter((float)left, (float)right,
        ...
        {
            int errorCode;
            GL.GetProgram(programId, ProgramParameter.LinkStatus, out errorCode);
            return errorCode;
        }
        public void ActiveTexture(int paramIndex) { GL.ActiveTexture(TextureUnit.Texture0 + paramIndex); }
        public int GetMaxTextureUnits() { int result; GL.GetInteger(GetPName.MaxTextureImageUnits, out result); return result; }
